...
const AdminGuestsPage: React.FC = () => {
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setImportResult({ message: t('ad_import_success', {inserted: result.inserted, updated: result.updated}), success: true });
            // In a real app, you would re-fetch the guest list here to show the new data
        } catch (error: any) {
            setImportResult({ message: error.message || t('error.generic'), success: false });
        } finally {
            setImporting(false);
