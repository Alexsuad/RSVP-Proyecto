...
from typing import (
    ...
    List,
    Literal,
)  # Importa tipos para anotar opcionales, listas y literales.
...
LanguageLiteral = Literal[
    "es", "en", "ro"
]  # Define el conjunto de valores válidos para 'lang'.
...
def _normalize_phone(
    raw: Optional[str],
) -> Optional[str]:  # Normaliza teléfonos entrantes.
    ...
    digits = re.sub(
        r"[^\d+]", "", raw.strip()
    )  # Elimina cualquier cosa que no sea dígito o '+'.
...
class GuestBase(
    BaseModel
):  # Modelo base para invitados (compartido entre requests/responses).
    ...
    language: Optional[LanguageEnum] = (
        None  # 🔁 Idioma preferido (opcional para no forzar EN por defecto).
    )
    invite_type: InviteTypeEnum = (
        InviteTypeEnum.full
    )  # Tipo de invitación (full/reception-only).
    max_accomp: int = Field(default=0, ge=0)  # Máximo de acompañantes permitidos (>=0).

    confirmed: Optional[bool] = (
        None  # Confirmación de asistencia (puede ser None si no ha respondido).
    )
...
class GuestResponse(GuestBase):  # Modelo de respuesta pública del invitado (protegida).
    ...
    invited_to_ceremony: bool | None = (
        None  # Campo calculado: asiste a ceremonia (lo rellena el router).
    )
    invite_scope: str | None = (
        None  # Campo calculado: 'ceremony+reception' o 'reception-only'.
    )
    model_config = ConfigDict(
        from_attributes=True, use_enum_values=True
    )  # ⬅️ Exporta enums por su value ("es","en","ro").
...
class CompanionIn(
    BaseModel
):  # Modelo de entrada para acompañantes (desde el invitado).
...
class CompanionOut(
    CompanionIn
):  # Modelo de salida para acompañantes (mismo shape que entrada).
    model_config = ConfigDict(
        from_attributes=True
    )  # No añade campos extra (permite extender si se requiere).
...
class LoginRequest(BaseModel):  # Modelo para login clásico (guest_code + email/phone).
    ...
    def _check_email_or_phone(
        self,
    ):  # Exige al menos un contacto válido además del código.
        self.email = self.email or None  # Normaliza email nulo.
        self.phone = _normalize_phone(self.phone)  # Normaliza teléfono si vino.
        if (self.email is None) and (self.phone is None):  # Si ambos están ausentes...
            raise ValueError(
                "Debes proporcionar email o teléfono para iniciar sesión."
            )  # ...lanza error claro.
        self.guest_code = (
            self.guest_code or ""
        ).strip()  # Asegura que el código no esté vacío/espaciado.
...
class RecoveryRequest(BaseModel):  # Modelo para recuperar el código por email/teléfono.
    ...
    phone: Optional[str] = None  # Teléfono opcional.
    lang: Optional[str] = (
        None  # 🆕 Idioma sugerido por el cliente (se pasará al resolver).
    )
    ...
    def _check_contact(self):  # Exige al menos un contacto.
        ...
        if (self.email is None) and (self.phone is None):  # Si no hay ninguno...
            raise ValueError(
                "Debes indicar email o teléfono para recuperar tu código."
            )  # ...lanza error claro.
...
class RequestAccessPayload(
    BaseModel
):  # Payload para solicitar acceso (código o magic link).
    full_name: str = Field(
        ..., min_length=3, max_length=120
    )  # Nombre completo (validación mínima).
    phone_last4: str = Field(
        ..., min_length=4, max_length=4
    )  # Últimos 4 del teléfono (exactamente 4).
    email: EmailStr  # Correo a donde se enviará el acceso.
    consent: bool = True  # Consentimiento para comunicaciones (GDPR-friendly).
    lang: Optional[LanguageLiteral] = (
        None  # 🆕 Idioma canónico del payload (opcional, sin default).
    )
    preferred_language: Optional[LanguageLiteral] = (
        Field(  # 🆕 Alias de compat. para clientes que envían este nombre.
    ...
    def _merge_lang_alias(self):  # Fusiona alias → campo canónico.
        self.lang = (
            self.lang or self.preferred_language or None
        )  # Si 'lang' no vino, usa 'preferred_language'.
        return self  # Devuelve la instancia validada.
    ...
    def _only_digits_len4(cls, v: str) -> str:  # Enforce: 4 dígitos exactos.
        ...
        ):  # Comprueba longitud 4 y que sean dígitos.
            raise ValueError(
                "phone_last4 debe tener exactamente 4 dígitos numéricos."
            )  # Error claro si no cumple.
        return v  # Devuelve valor válido.


class MagicLoginPayload(
    BaseModel
):  # Payload para canjear token mágico por access token.
...
# =================================================================================
class RSVPUpdateRequest(
    BaseModel
):  # Modelo para actualizar la respuesta RSVP del invitado.
    ...
    )  # Notas opcionales, limitadas a 500 caracteres.
    ...
    companions: Optional[List[CompanionIn]] = (
        None  # Acompañantes (opcional para permitir updates parciales).
    )
    ...
    def _sanitize_fields(self):  # Limpia/trunca campos libres para evitar excesos.
        self.allergies = (
            self.allergies or ""
        ).strip() or None  # Normaliza alergias a None si vacío.
...
# =================================================================================
class GuestWithCompanionsResponse(
    GuestResponse
):  # Respuesta que incluye invitado + acompañantes.
    companions: List[CompanionOut] = Field(
        default_factory=list
    )  # Lista de acompañantes (vacía por defecto).
    model_config = ConfigDict(
        from_attributes=True, use_enum_values=True
    )  # ⬅️ Exporta enums por su value en JSON.
...
class GuestUpdate(BaseModel):
    ...
    rsvp_status: Optional[str] = (
        None  # Para modificar estado manualmente si fuera necesario
    )
